...
class Client(httpx.AsyncClient):
    """
    An async client to retrieve data from the Clash Royale API.

    Create a client instance with the given options.

        >>> from clash_royale.aio import Client
        >>> async with Client(api_key="your_api_key_here") as client:
        ...     player = await client.players.get("#ABC123")
    ...
    by setting the `proxy` parameter.
    ...
    async def _request(
        ...
        try:
            response = await super().request(method, endpoint, params=params)
