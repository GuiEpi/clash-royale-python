...
class Leaderboards(Resource):
    """
    Async resource for leaderboard-related endpoints.

    Check the :clash-royale-api:`leaderboards`
    for more detailed information about each endpoint.
    """

    async def list(self) -> list[Leaderboard]:  # ty:ignore[invalid-type-form]
        """List all available leaderboards."""
        response = await self._client._request("GET", "/leaderboards")
